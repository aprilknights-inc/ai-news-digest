...
| ファイル | 役割 | 触る？ |
|---|---|---|
| `sources.md` | 巡回する情報源・Xアカ・6セクションの方針の**正本** | ◎ ここを編集して好みに調整 |
| `style.css` | 全ページ共有のデザイン（崩さない） | △ 見た目を変えたい時だけ |
| `app.js` | 保存（お気に入り）・共有（LINE/共有シート/コピー）の挙動 | △ |
| `template.html` | 日次ページの雛形（6セクション） | ✕ ルーティンが使う |
| `favorites.html` | 保存したニュース一覧（端末ブラウザ内に保存） | ✕ |
